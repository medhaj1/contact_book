...
const Dashboard = ({ currentUser, onLogout = () => {} }) => {
  // Read preferences from localStorage with fallback defaults
  ...
  // --- Name and Date Formatting Utilities ---
  const formatName = (contact) => {
    ...
    if (isFirstLast) {
      // Display exactly as stored in Supabase
      return name;
    }
    // Display as "Last, First" only when preference is last_first
    const parts = name.split(" ").filter(Boolean);
    if (parts.length < 2) return name; // single-word name stays the same
  ...
  // Debug logs
  console.log("Contacts:", contacts.map(c => c.contact_id));
  ...
  const todaysBirthdays = unblockedContacts.filter((c) => isBirthdayToday(c.birthday));
  ...
  return (
    <div className="flex min-h-screen font-sans">
      ...
      <div className="flex-1 p-8 bg-blue-50 dark:bg-[#0d1117] ml-16">
          ...
          <div className="relative flex items-center gap-3">
            ...
            {showUserDropdown && (
              <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-[#161b22] rounded-xl shadow-lg border border-slate-200 dark:border-[#30363d] py-1 z-50">
                ...
                <div
                  className="flex items-center mx-1 px-4 py-2 text-sm rounded-lg text-slate-700 dark:text-[#c9d1d9] hover:text-blue-700 dark:hover:font-semibold dark:hover:text-indigo-200 hover:bg-blue-100 dark:hover:bg-indigo-700/30 cursor-pointer"
                  onClick={() => {
                    navigate("/settings");
                    setShowUserDropdown(false);
                  }}
                >
                  <Settings size={16} className="mr-2 inline" /> Settings
                </div>
                <div
                  className="flex items-center mx-1 px-4 py-2 text-sm rounded-lg text-slate-700 dark:text-[#c9d1d9] hover:text-red-700 dark:hover:font-semibold dark:hover:text-red-200 hover:bg-red-100 dark:hover:bg-red-700/30 cursor-pointer"
                  onClick={onLogout}
                >
        ...
        {activeTab === "contacts" && (
          <>
              ...
              <div className="mb-6 p-4 bg-amber-50 rounded-xl border border-amber-200 flex flex-col gap-2">
                ...
                {todaysBirthdays.map((contact) => (
                  <div key={contact.contact_id} className="flex items-center gap-3 text-amber-700">
                    <span className="font-bold">{formatName(contact)}</span>
                    <span className="text-xs">(Today)</span>
                    <span className="text-xs text-slate-500 ml-1">{formatDate(contact.birthday)}</span>
            ...
            <BirthdayReminder contacts={unblockedContacts} formatName={formatName} formatDate={formatDate} />
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full bg-blue-100 dark:bg-indigo-500 flex justify-center items-center font-bold overflow-hidden">
                            {c.photo_url ? (
                              <img src={c.photo_url} alt={formatName(c)} className="w-full h-full object-cover" />
                            ) : (
                              safeString(formatName(c)).charAt(0).toUpperCase()
                            )}
                          </div>
                          <div>
                            <h3 className="text-lg font-bold">{formatName(c)}</h3>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="text-lg font-bold text-slate-900 dark:text-slate-200">{formatName(c)}</div>
                          {renderCategoryBadges(c)}
      ...
      {/* Floating Add Contact Button (FAB) with hover/expanded logic */}
      {activeTab === "contacts" && (
