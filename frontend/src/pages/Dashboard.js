...
const handleDeleteDocument = async (doc) => {
  ...
  return (
    <div className="flex min-h-screen font-sans">
      ...
      <div className="flex-1 ml-60 p-8 bg-blue-50 dark:bg-[#0d1117] transition-all duration-200 min-h-screen">
        {/* Header */}
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="relative flex items-center gap-3">
            ...
            >
              ...
              <span className="text-sm text-slate-600 dark:text-[#c9d1d9] font-medium">{userName}</span>
            ...
            {showUserDropdown && (
              <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-[#161b22] rounded-xl shadow-lg border border-slate-200 dark:border-[#30363d] py-1 z-50">
                <div
                  className="flex items-center mx-1 px-4 py-2 text-sm rounded-lg text-slate-700 dark:text-[#c9d1d9] hover:text-blue-700 dark:hover:font-semibold dark:hover:text-indigo-200 hover:bg-blue-100 dark:hover:bg-[#23272f] cursor-pointer"
                  onClick={() => {
                    navigate('/profile');
                    setShowUserDropdown(false);
                  }}
                ...
                <div
                  className="flex items-center mx-1 px-4 py-2 text-sm rounded-lg text-slate-700  dark:text-[#c9d1d9] dark:hover:font-semibold hover:text-red-600 dark:hover:text-red-200 dark:hover:bg-red-800 hover:bg-red-100 cursor-pointer"
                  onClick={() => {
                    onLogout();
                    setShowUserDropdown(false);
                  }}
        ...
        {activeTab === 'contacts' && (
          <>
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 rounded-xl dark:text-[#c9d1d9] border border-blue-100 dark:bg-[#161b22] dark:border-[#30363d] text-sm focus:outline-none hover:border-blue-200 dark:hover:border-[#30363d] shadow focus:ring-1 focus:ring-blue-100 dark:focus:ring-indigo-500 transition-colors"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
              </div>
              <select
                className="pl-6 pr-3 py-2 rounded-xl dark:text-[#c9d1d9] dark:bg-[#161b22] border border-blue-100 dark:border-[#30363d] text-md focus:outline-none hover:border-blue-200 dark:hover:border-[#30363d] hover:shadow focus:ring-1 focus:ring-blue-100 dark:focus:ring-indigo-500 transition-colors"
            ...
            {todaysBirthdays.length > 0 && (
              <div className="mb-6 p-4 bg-sky-200 bg-opacity-60 dark:bg-[#161b22] dark:bg-opacity-100 rounded-xl shadow-md hover:shadow-lg border border-sky-200 dark:border-[#30363d] flex flex-col gap-2">
            ...
            {/* Contact Cards */}
            {loading ? (
              <div className="text-center text-slate-400 dark:text-[#c9d1d9] py-12">
                Loading contacts...
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredContacts.map(contact => (
                  <div
                    ...
                    className="bg-white dark:bg-[#161b22] dark:bg-opacity-100 border border-blue-100 dark:border-[#30363d] dark:border-opacity-60 dark:hover:shadow-[#161b22] p-6 rounded-2xl transition hover:shadow-md hover:-translate-y-1"
                  >
                    <div className="flex items-center mb-4">
                      <div className="w-12 h-12 rounded-full bg-blue-100 dark:bg-[#2a2a2a] flex items-center justify-center text-blue-700 dark:text-indigo-700 font-semibold text-lg overflow-hidden mr-4">
                        {contact.photo_url
                          ? <img src={contact.photo_url} alt={contact.name} className="w-full h-full object-cover rounded-full" />
                          : contact.name.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <h3 className="text-lg font-bold text-slate-900 dark:text-[#c9d1d9]">{contact.name}</h3>
                        {contact.category_id && (
                          <span className="text-xs bg-blue-100 dark:bg-[#23272f] text-blue-800 dark:text-[#c9d1d9] px-2 py-0.5 rounded-full font-medium">
                    ...
                    <div className="flex items-center gap-2 text-slate-500 dark:text-[#c9d1d9] text-sm mb-1">
                      <Mail size={16} />
                    ...
                    <div className="flex gap-2 mt-4">
                      <button
                        className="p-2 bg-blue-500 hover:bg-blue-600 dark:bg-indigo-500 text-white rounded-md"
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {categories.map(category => (
                <div key={category.category_id} className="bg-white dark:bg-[#161b22] border border-blue-100 dark:border-[#30363d] p-6 rounded-2xl flex justify-between items-center">
                  <div>
                    <h3 className="text-lg font-semibold text-slate-900 dark:text-[#c9d1d9]">{category.name}</h3>
                    <p className="text-sm text-slate-500 dark:text-[#808080] mt-1">
                      {contacts.filter(c => String(c.category_id) === String(category.category_id)).length} contacts
                    </p>
                  </div>
                  <button
                    className="p-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-md hover:bg-red-100 dark:hover:bg-red-900/40 ml-4"
        ...
        {activeTab === 'documents' && (
          <>
            {/* Upload Section */}
            <div className="mb-8">
              <div className="bg-white dark:bg-[#161b22] border border-blue-100 dark:border-[#30363d] p-6 rounded-2xl">
                <h3 className="text-lg font-semibold text-slate-900 dark:text-[#c9d1d9] mb-4">Upload Documents</h3>
                <div className="relative border-2 border-dashed border-blue-200 dark:border-[#30363d] rounded-xl p-8 text-center hover:border-blue-300 dark:hover:border-[#23272f] transition-colors">
                  <div className="flex flex-col items-center gap-4">
                    <div className="w-16 h-16 bg-blue-100 dark:bg-[#23272f] rounded-full flex items-center justify-center">
                      <svg className="w-8 h-8 text-blue-600 dark:text-[#808080]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-slate-600 dark:text-[#c9d1d9] mb-2">
                        <span className="font-medium">Click to upload</span> or drag and drop
                      </p>
                      <p className="text-sm text-slate-500 dark:text-[#808080]">
            ...
            {/* Documents List */}
            <div>
              <h3 className="text-lg font-semibold text-slate-900 dark:text-[#c9d1d9] mb-4">
                Uploaded Documents ({documents.length})
              </h3>
              {documents.length === 0 ? (
                <div className="text-center py-12">
                  <div className="w-16 h-16 bg-[#E0E0E0] dark:bg-[#161b22] rounded-full flex items-center justify-center mx-auto mb-4">
                    <BookOpen size={32} className="text-slate-400 dark:text-[#808080]" />
                  </div>
                  <p className="text-slate-400 dark:text-[#808080] text-lg mb-2">No documents uploaded yet</p>
                  <p className="text-slate-500 dark:text-[#A6A6A6] text-sm">Upload your first document to get started</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {documents.map(doc => (
                    <div key={doc.id} className={`bg-white dark:bg-[#161b22] border border-blue-100 dark:border-[#30363d] p-6 rounded-2xl transition hover:shadow-md hover:-translate-y-1`}>
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <div className="w-10 h-10 bg-blue-100 dark:bg-[#23272f] rounded-lg flex items-center justify-center flex-shrink-0">
                            {doc.name.toLowerCase().includes('.pdf') ? (
                              ...
                              <svg className="w-6 h-6 text-slate-600 dark:text-[#808080]" fill="currentColor" viewBox="0 0 20 20">
                                <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd" />
                              </svg>
                            )}
                          </div>
                          <div className="min-w-0 flex-1">
                            <h4 className="text-sm font-medium text-slate-900 dark:text-[#c9d1d9] truncate" title={doc.name}>
                              {doc.name}
                            </h4>
                            <p className="text-xs text-slate-500 dark:text-[#A6A6A6] mt-1">
                      ...
                      <div className="flex gap-2">
                        <a
                          ...
                          className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-blue-50 dark:bg-[#23272f] text-blue-600 dark:text-blue-400 rounded-lg text-sm hover:bg-blue-100 dark:hover:bg-[#30363d] transition-colors"
                        >
